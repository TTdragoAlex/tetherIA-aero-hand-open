...
HARDWARE_01_REAL_TUNED_BALL45_ACTION_MODE = "hardware_01_real_order_real_tuned_ball45"
...
def hardware_01_real_tuned_ball45_config() -> config_dict.ConfigDict:
  """4.5 cm ball variant using the current real-tuned command window.

  This keeps the best real-hand-fitted command window but replaces the cube XML
  with a 45 mm sphere. The object/body/sensor names stay as cube internally so
  existing reward and export tooling continues to work.
  """
  config = hardware_01_real_tuned_window_config()
  config.action_mode = HARDWARE_01_REAL_TUNED_BALL45_ACTION_MODE
  config.cube_start_pos = [0.105, 0.0, 0.045]
  config.cube_start_pos_jitter = [0.018, 0.018, 0.010]
  config.reward_config.scales.linvel = -0.18
  config.reward_config.scales.cube_planar_drift = -0.65
  config.reward_config.scales.thumb_index_trap = -0.35
  config.reward_config.scales.thumb_index_pinch = -0.30
  config.reward_config.scales.ring_pocket_trap = -0.18
  config.reward_config.scales.tuned_command_window = -0.25
  config.reward_config.scales.coordinated_finger_participation = 0.08
  config.reward_config.scales.index_ring_participation = 0.08
  return config
...
class CubeRotateZAxis(aero_hand_base.AeroHandEnv):
  ...
  def _uses_hardware_01_action_mode(self) -> bool:
    return self._config.get("action_mode", "sim_delta") in (
        ...
        HARDWARE_01_REAL_TUNED_BALL45_ACTION_MODE,
    )
...
class Ball45mmRotateZAxisHardware01RealTunedWindow(CubeRotateZAxis):
  """Rotate a 45 mm ball using the current real-tuned hardware command window."""

  def __init__(
      self,
      config: config_dict.ConfigDict = hardware_01_real_tuned_ball45_config(),
      config_overrides: Optional[Dict[str, Union[str, int, list[Any]]]] = None,
  ):
    aero_hand_base.AeroHandEnv.__init__(
        self,
        xml_path=consts.BALL_45MM_XML.as_posix(),
        config=config,
        config_overrides=config_overrides,
    )
    self._post_init()
